...
Component({
  ...
  methods: {
    ...
    setMonth(setYear, setMonth, setDay) {
      console.log(setMonth + "setMonth==============" + setDay)
    ...
    selectChange(e) {
      ...
      var _selday = this.changeStrToTimestamp(dateString) / 24 / 3600 / 1000
      var today = Date.parse(new Date()) / 24 / 3600 / 1000;
      if (_selday > today){

      }else{
        return
      }
      // console.log(_selday + " == = = = " + today + "  -- " + str)
      ...
      if (this.data.selectDay.year !== year || this.data.selectDay.month !== month) {
        ...
        if (this.data.firstSelection){
          selectDay.type = "start"
          this.setData({
            selectDayStart: selectDay,
            firstSelection: false,
            selectDayEnd: {},
          })
        } else {
          var daysStart = this.changeStrToTimestamp(this.data.selectDayStart.dateString) / 24 / 3600 / 1000
          var daysEnd = this.changeStrToTimestamp(dateString) / 24 / 3600 / 1000
          if (daysStart < daysEnd){
            selectDay.type = "end"
            this.setData({
              selectDayEnd: selectDay,
              firstSelection: true
            })
          }
        }
    ...
    changeStrToTimestamp:function(_str){
        ...
        console.log("dsfsd " + newDate.getTime());
        return newDate
      },
  },
  lifetimes: {
